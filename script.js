        ...
        checkbox.addEventListener('change', function () {
            ...
            if (drugs.includes(drug)) {
                if (this.checked) {
                    ...
                    if (!Array.from(selectedDrugsContainer.children).some(el => el.textContent.trim() === drug.trim())) {
                        selectDrug(drug);
                    }
                } else {
...
// ПОДГРУЖАЕМ ДАННЫЕ
// ---------------------------------------------------
let reviewsData = []; // Глобальная переменная для хранения данных
...
function loadCSVData() {
    Papa.parse("data/results.csv", {
        ...
        complete: function(results) {
            reviewsData = results.data; // Сохраняем данные в глобальную переменную
            console.log("Data loaded:", reviewsData);
            filterReviews(); // Применяем фильтры после загрузки данных
        }
    });
}
...
function updateTable(filteredReviews) {
    ...
    document.getElementById('reset-button').addEventListener('click', function () {
        ...
        // Сброс поиска
        const selectedDrugsContainer = document.getElementById('selected-drugs');
        selectedDrugsContainer.innerHTML = ''; // Очищаем выбранные лекарства

        // Восстанавливаем исходный список лекарств
        drugs.length = 0; // Очищаем текущий список
        drugs.push(
            'Авиандр',
            'Алимемазин и синонимы',
            'Андипал',
